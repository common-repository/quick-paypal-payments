...
class Admin_Pages {
	...
	public function settings_setup() {
		...
		*/
		...
		add_submenu_page(
			...
			$this->settings_title,                       /* Menu Title */
			'manage_options',                 /* Capability */
			'quick-paypal-payments-settings',                         /* Page Slug */
			array( $this, 'settings_page' )          /* Settings Page Function Callback */
		);
	...
	public function footer_scripts() {
		$page_hook_id = $this->settings_page_id;
		$confmsg      = __( 'Are you sure want to do this?', 'quick-paypal-payments' );
		?>
...
// confirm before reset
                $('#delete-action input').on('click', function () {
                    return confirm('<?php echo wp_kses_post( $confmsg ); ?>');
                });
            });
            //]]>
        </script>
	...
	public function add_required_meta_boxes() {
		...
		if ( $this->settings_page_id == $hook_suffix ) {
			...
			add_meta_box(
				...
				__( 'Save Options', 'quick-paypal-payments' ),            /* Title */
        ...
        <div id="submitpost" class="submitbox">
            <div id="major-publishing-actions">
                ...
                <div id="publishing-action">
					...
					<?php submit_button( esc_attr__( 'Save', 'quick-paypal-payments' ), 'primary', 'submit', false ); ?>
                </div>
                <div class="clear"></div>
	...
	public function reset_sanitize( $settings ) {
		/* Add Update Notice */
		if ( ! empty( $settings ) ) {
			add_settings_error( $this->option_group, '', esc_html__( 'Settings reset to defaults.', 'quick-paypal-payments' ), 'updated' );
			/* Delete Option */
			$this->delete_options();
		}
